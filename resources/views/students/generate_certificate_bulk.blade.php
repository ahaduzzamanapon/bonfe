...
<head>
    ...
    <style>
        ...
        .certificate {
            margin: auto;
            padding: 75px 50px;
            background-image: url('{{ asset('images/certificate_background.jpg') }}');
            /* Adjust as needed */
...
<body>
    ...
    <div class="certificate">
        ...
        @endphp
        <div class="seal">
            <img src="{{ !empty($setting) ? asset($setting->logo) : 'assets/images/Picture1.jpg' }} "alt="Logo"
                style="width: 168px;height: 169px;"> <!-- Replace with dynamic logo -->
        </div>
        <div class="header">
        ...
        <div class="content">
            <p>
                This is to certify that <strong>{{ $student->candidate_name }}</strong> Mother's Name:
                <strong>{{ $student->mother_name }}</strong> & Father's Name:
                <strong>{{ $student->father_name }}</strong>, Date of Birth:
                <strong>{{ $student->date_of_birth }}</strong> Birth Registration No.
                <strong>{{ $student->nid }}</strong> has been awarded prevocational on
                <strong>{{ $student->occupation }}</strong> for successful completion of the following competencies
                under Bangladesh National Qualification Framework (BNQF):
            </p>
